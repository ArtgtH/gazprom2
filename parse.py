...
df = (pd.DataFrame(Google_Table()[0].get_all_records()))[['Наименование сценария', 'Описание', 'Домен','Технология','Метод использования','Функциональная группа', 'Наименование | Бенчмаркинг (внешний рынок)', 'Описание | Бенчмаркинг (внешний рынок)', 'Описание проекта в ГПН | НИОКР', 'Описание проекта в ГПН | НИОКР', 'Название проекта | Проекты ЦТ', 'Потенциал решения''Рыночная зрелость','Организационная готовность','Реализуется в Газпром нефти?']]
m = Mystem()
